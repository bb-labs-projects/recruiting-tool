...
- On drop/select: immediately starts uploading each file to Vercel Blob using:
  ...
  ```
- After each successful blob upload, create the cvUpload DB record via PUT to the upload route:
  ```typescript
  ...
  ```
  This works in both local dev and production. The PUT handler on the upload route (created in Plan 02-03) creates the DB record.
