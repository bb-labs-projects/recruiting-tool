...
Output: Five route handler files under src/app/api/admin/cv/.
</objective>
...
<task id="02-03-T1">
<title>Task 1: Create Vercel Blob upload route and status endpoints</title>
<details>
Create three route files:

**File 1: `src/app/api/admin/cv/upload/route.ts`**
...
import { verifyAdminSession } from './auth-helper' // see below
```

Implementation:
- Export POST handler
- Verify admin auth using a helper (see auth pattern below)
- Call `handleUpload()` with:
  - `body`: parsed from request JSON
  - `request`: the incoming request
  - `onBeforeGenerateToken`: async callback that verifies admin auth (return `{ allowedContentTypes: ['application/pdf'], maximumSizeInBytes: 10 * 1024 * 1024 }` for 10MB limit)
  - `onUploadCompleted`: async callback that creates a cvUpload record in the database with `{ filename: blob.pathname, blobUrl: blob.url, uploadedBy: adminUserId, status: 'uploaded' }`. Note: this callback does NOT fire in local dev (Vercel servers can't reach localhost), so also handle record creation client-side as a fallback.
- Return the JSON response from handleUpload

**Auth helper pattern** (inline in each route, not a shared file -- keeps routes self-contained):

For API routes, we cannot use the DAL's `verifySession()` because it calls `redirect()` which throws. Instead, create a helper function at the top of each route file:
...
Actually -- to avoid duplicating this in every route, create a shared helper file:
...
**File 2: `src/app/api/admin/cv/status/route.ts`**
...
**File 3: `src/app/api/admin/cv/parse/[id]/route.ts`**
...
2. All five route files exist and export the correct HTTP method handlers
