...
<task type="auto">
  ...
  <action>
    1. Run `npx create-next-app@latest . --typescript --tailwind --eslint --app --src-dir --import-alias "@/*"` in the project root. Accept defaults. If the project root already has files (like .planning, .claude), the CLI may error -- in that case, create in a temp directory and move files, or use `--yes` flag. The key is to end up with the standard Next.js 16 project structure in the current repo root.
  ...
  <done>
    Next.js dev server runs successfully. Tailwind v4 is configured via CSS. shadcn/ui components are installed. All auth-related npm packages are in package.json. Environment variable template exists.
  </done>
</task>
...
- Tailwind v4 CSS-first config working
